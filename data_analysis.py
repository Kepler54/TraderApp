...
class DataAnalysis(DataBase):
    ...
    def buy_sell_verify(self) -> tuple[str, str]:
        ...
        :return: tuple
        """
        if (float(self.percentage(self.coin_trade_info(self.sell)[-1], self.sell)[0]) >=
                float(self.get_value_list("SELECT notification FROM trades")[0][0])):
            return f", нужно покупать [{self.get_coin_name(self.coin_second)}]", "notification"
        elif (float(self.percentage(self.coin_trade_info(self.buy)[-1], self.buy)[0]) <=
              -float(self.get_value_list("SELECT notification FROM trades")[0][0])):
